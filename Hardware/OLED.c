...
void* OLED_get_canvas_buffer(void){
    return OLED_DisplayBuf;
}

void SCL_ON(void){
    GPIO_SetBits(OLED_GPIOx, OLED_SCL_PIN);
}
void SCL_OFF(void){
    GPIO_ResetBits(OLED_GPIOx, OLED_SCL_PIN);
}
void SDA_ON(void){
    GPIO_SetBits(OLED_GPIOx, OLED_SDA_PIN);
}
void SDA_OFF(void){
    GPIO_ResetBits(OLED_GPIOx, OLED_SDA_PIN);
}
...
{
    SDA_ON();		//释放SDA，确保SDA为高电平
    SCL_ON();		//释放SCL，确保SCL为高电平
    SDA_OFF();		//在SCL高电平期间，拉低SDA，产生起始信号
    SCL_OFF();		//起始后把SCL也拉低，即为了占用总线，也为了方便总线时序的拼接
...
{
    SDA_OFF();		//拉低SDA，确保SDA为低电平
    SCL_ON();		//释放SCL，使SCL呈现高电平
    SDA_ON();		//在SCL高电平期间，释放SDA，产生终止信号

//    I2C_GenerateSTOP(I2C2, ENABLE);	//产生终止信号
}
...
void OLED_I2C_SendByte(uint8_t Byte)
{
	...
	for (i = 0; i < 8; i++)
	{
        ...
        SCL_ON();	//释放SCL，从机在SCL高电平期间读取SDA
        SCL_OFF();	//拉低SCL，主机开始发送下一位数据
	}

    SCL_ON();		//额外的一个时钟，不处理应答信号
    SCL_OFF();
...
/**
  ...
  *                 OLED_6X8		宽6像素，高8像素
  * @return 无
  * @note 显示的中文字符需要在OLED_Data.c里的OLED_CF16x16数组定义
		...
		for (i = 0; i < Width; i ++)
		{
			if (X + i >= 0 && X + i <= 127)		//超出屏幕的内容不显示
			{
				...
				if (Page + j >= 0 && Page + j <= 7)		//超出屏幕的内容不显示
				{
					...
					} else if (color == 2) { // XOR模式：异或操作
						OLED_DisplayBuf[Page + j][X + i] ^= Image[j * Width + i] << (Shift);
					} else if (color == 0) { // 背景色：清除对应位
						OLED_DisplayBuf[Page + j][X + i] &= ~(Image[j * Width + i] << (Shift));
					}
				}
...
void OLED_ShowImage(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
	...
	OLED_PrintImage(X, Y, Width, Height, Image);
}
...
void OLED_DrawPoint(int16_t X, int16_t Y)
{
	if (X >= 0 && X <= 127 && Y >=0 && Y <= 63)		//超出屏幕的内容不显示
	{
        ...
        } else if (color == 1) { // 前景色：设置像素
            OLED_DisplayBuf[Y / 8][X] |= 0x01 << (Y % 8);
        } else if (color == 2) { // XOR模式：反转像素
            OLED_DisplayBuf[Y / 8][X] ^= 0x01 << (Y % 8);
        }
	}
}
 ...
 * @param x,y      矩形左上角坐标
 * @param w,h      矩形宽度和高度
 * @param r        圆角半径（自动限制为 ≤ w/2 且 ≤ h/2）
 * @param isFilled 是否填充（0=仅描边，1=填充）
 */
void OLED_DrawRoundedRect(int16_t x, int16_t y, uint16_t w, uint16_t h, uint8_t r, uint8_t isFilled) {
    ...
    // 计算矩形边界
    int16_t x1 = x + w - 1;  // 右下角X坐标
    int16_t y1 = y + h - 1;  // 右下角Y坐标

    // 绘制四条边（避开圆角区域）
    OLED_DrawLine(x+r, y, x1-r, y);             // 上边
    OLED_DrawLine(x+r, y1, x1-r, y1);           // 下边
    OLED_DrawLine(x, y+r, x, y1-r);             // 左边
    OLED_DrawLine(x1, y+r, x1, y1-r);           // 右边
    // 填充模式：绘制完整填充的圆角矩形
    if (isFilled) {
        OLED_DrawRectangle(x+r, y, w-2*r, r, 1); // 填充上边和下边
        OLED_DrawRectangle(x, y+r, w, h-2*r, 1); // 填充左边和右边
        OLED_DrawRectangle(x+r, y+h-r, w-2*r, r, 1); // 填充内部区域
    }
    // 绘制四个圆角轮廓
    OLED_DrawArc(x+r, y+r, r, -180, -90, isFilled);    // 左上角
    OLED_DrawArc(x1-r, y+r, r, -90, 0, isFilled);      // 右上角
    OLED_DrawArc(x+r, y1-r, r, 90, 180, isFilled);     // 左下角
    OLED_DrawArc(x1-r, y1-r, r, 0, 90, isFilled);      // 右下角
}
