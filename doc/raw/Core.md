...
> （当你打开keil时，你会发现确实一大堆文件，没事，你把那些东西从keil文件列表里删掉就好，我这还要用就懒得删了。。）
...
（当你打开keil时，你会发现确实一大堆文件找不到，没事，你把那些东西从keil文件列表里删掉就好，我这还要用就懒得删了。。）
...
例如：
```c
UI_AddTask(GetTick()+3000,task_func,component)->finish = 1;
```
这句话向三秒后部署了运行`task_func`这一函数的任务，当该函数被调用时，将会把`component`作为参数传入，具体行为由函数本身决定。
`->finish = 1`在这里表示在任务运行之后将销毁这个task对象。
...
```c
UI_AddTask(0,task_func,component)->finish = 0;
```
我将任务部署在时间线的最开始，意味着每一次检查这个task时都会被执行，而`finish`参数始终为0，这个任务将不会被删除。
...
我们有以下实践来解决这个问题：\
（在看之前，请确保你的函数**能**够被转化成`void`返回，0或1个参数传入的函数）
假定我们有一个大工作流函数如下：
```c
...
```
1. 为函数添加一个表示状态的静态参数，并添加一个承接事件对象的指针。
```c
static int8_t state = 0;
static UITask* t = 0;
```
2. 将函数中所有操作都用如下结构包裹起来
```c
switch(state){
    ...
}
```
3. 找出所有函数中的等待步骤，如等待标志位，等待时间等，在**这些地方前**和**函数开头**插入`case x:  ...  return;`结构。\
在第一个case后面完成对函数内task的初始化，方便以后调用。在此之前你需要确保函数符合传入一个指针，返回空的格式。\
在每一个return前插入state的状态更新。即`state = x`

4. 写一个外层函数，这个函数与原函数的接口相同，即相同函数名，相同参数列表，相同返回值，用于向外暴露接口。\
将原函数名改一个，例如`_demo`，原函数传入参数为`void* arg`，返回`void`。

5. 将`while(x)`等待改为`if(x) return;`，将`delay(x)`改为`t->process_time = GetTick() + x;`。\
将函数中被调用的参数强制转化成所需的类型。
一般情况下这个任务只会被执行一轮，故需要将这个task销毁掉，因此需要在最后一行前加上`t->finish = 1;`

6. 在外层函数中调用这个改造后的函数。同时为了避免函数被同时调用，需要加一个全局互斥锁，例如：`uint8_t demo_lock = 0;`,并在外层函数中进行上锁和检查，内层函数中解锁。

完整如下：
```c
uint8_t demo_lock = 0;   //这是一个互斥锁
...
void _demo(void* arg){
    ...
    switch(state){
        case 0:
            t = UI_AddTask(0,_demo,0);  //注意！这里的函数名是修改过的！
            action1();
            state = 1;
        return;
        case 1:
            if(isready() == 0) return;  //等待某一个标志位准备就绪
            action2((Object*)obj);  //需要进行强制类型转换，否则编译器不认
            state = 2;
        return;
        case 2:
            t->process_time = GetTick() + 1000;  //等待一秒
            action3();
            state = 0;
...
void demo(Object* obj){
    if(demo_lock == 1) return;  //已上锁，返回
    demo_lock = 1;    //上锁
    _demo((void*)obj)
}
```
到此，这个函数就可以在异步框架中被顺滑调用了！在等待时，仍然会进行动画更新屏幕刷新等操作。
这个方案其实漏洞还是蛮多的，但是能较大程度上解决部分问题。\
实际上多参数方案也不是没有，但是我就的还是有些臃肿，等我想想有没有什么比较我能接受的方案，后面再进行改进。

记录一下遗留的问题：
1. 如果想实现一个函数被多个任务调用，应该怎么改造呢？
2. 如果想传入多个参数，应该怎么写比较好呢？
3. 我还是觉得这个流程可以在直观一点，比如我想在哪截断，我就插上一条什么标签什么的。。感觉可以用宏封装？
